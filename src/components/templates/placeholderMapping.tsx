...
export const standardizedMappings = {
  ...
  // Nonmotor variations
  nonmotor: ["Nonmotor", "Non-motor", "Non Motor", "Nonmotor Index"],
 ...
 */
export const findStandardizedKey = (subtestName: string) => {
  ...
  for (const [normalizedTerm, standardKey] of normalizedMap.entries()) {
    if (
      ...
      normalizedSubtest.includes(normalizedTerm)
    ) {
      return standardKey;
    }
  }
  ...
  for (const [normalizedTerm, standardKey] of normalizedMap.entries()) {
    if (
      normalizedTerm.length > 2 &&
      normalizedTerm.includes(normalizedSubtest)
    ) {
      return standardKey;
    }
  }
  ...
  const subtestWords = subtestName
    .toLowerCase()
    .split(/\s+/)
    ...
    for (const [standardKey, terms] of Object.entries(standardizedMappings)) {
      for (const term of terms) {
        ...
        const overlap = subtestWords.filter((word) =>
          termWords.some(
            (termWord) => termWord.includes(word) || word.includes(termWord)
          )
        ).length;
        ...
        if (
          overlap > 0 &&
          overlap >= Math.min(subtestWords.length, termWords.length) * 0.5
        ) {
          return standardKey;
        }
      }
 ...
 */
export const createFallbackKey = (subtestName: string) => {
