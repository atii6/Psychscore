...
type SectionType = "basic" | "testBank" | "custom" | "conditional";
...
export default function PlaceholderPanel({
  ...
  const groupedTestBankPlaceholders = React.useMemo(() => {
    return testBankPlaceholders.reduce<Record<string, PlaceholdersType[]>>(
      (acc, placeholder) => {
        // Extract subtest name from placeholder: {{subtest_score}}
        const match = placeholder.placeholder.match(
          /{{(.+?)_(score|percentile|descriptor)}}/
        );

        const key = match ? match[1] : "other";

        if (!acc[key]) acc[key] = [];
        acc[key].push(placeholder);
  ...
  const handleInsertConditional = (exampleText: string) => {
    if (readOnly) return;
  ...
  return (
    <Card
      className="border-0 shadow-lg rounded-2xl sticky top-4"
      ...
      <CardContent className="space-y-4 max-h-96 overflow-y-auto">
              ...
              <PopoverContent className="w-80">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600">
                    Use conditional logic to show different text based on score
                    values.
                  </p>
                  ...
                  <p className="text-xs text-gray-600">
                    <strong>Example:</strong> If ASRS score ≥ 4, show "was
                    indicative", else show "was not indicative"
                  </p>
                </div>
              </PopoverContent>
            </Popover>
            ...
            <div className="space-y-2 ml-6">
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-xs font-medium mb-2">
                  Example: ASRS Score Interpretation
                </p>
            ...
            {expandedSections.basic && (
              <div className="space-y-2 ml-6">
                {basicPlaceholders.map((placeholder, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-2 rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50/30 transition-all"
                  >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        handleInsertPlaceholder(placeholder.placeholder)
                      }
          ...
          <div>
            ...
            {expandedSections.testBank && (
              <div className="space-y-3 ml-6">
                {Object.entries(groupedTestBankPlaceholders).map(
                  ([subtestKey, placeholders]) => {
                    if (subtestKey === "other") return null;

                    const subtestName =
                      placeholders[0]?.description
                        ?.replace(/(Score|Percentile|Descriptor)$/, "")
                        .trim() || subtestKey;
                    const scorePlaceholder = placeholders.find((p) =>
                      p.placeholder.includes("_score")
                    );
                    const percentilePlaceholder = placeholders.find((p) =>
                      p.placeholder.includes("_percentile")
                    );
                    const descriptorPlaceholder = placeholders.find((p) =>
                      p.placeholder.includes("_descriptor")
                    );
                        ...
                        <div className="grid grid-cols-3 gap-1">
                          ...
                          {percentilePlaceholder && (
                            <Button
                              ...
                              onClick={() =>
                                handleInsertPlaceholder(
                                  percentilePlaceholder.placeholder
                                )
                              }
                          ...
                          {descriptorPlaceholder && (
                            <Button
                              ...
                              onClick={() =>
                                handleInsertPlaceholder(
                                  descriptorPlaceholder.placeholder
                                )
                              }
        ...
        {!testType && Object.keys(groupedTestBankPlaceholders).length === 0 && (
          <div className="text-center py-4 text-sm text-gray-500">
            <p>
              Define subtests in the Test Bank for this test type to see score
              placeholders here.
            </p>
          </div>
        )}
