...
export default function ReportsPage() {
  ...
  const handleQuickViewReport = useCallback(async (assessmentId) => {
    try {
      const reports = await retryWithBackoff(async () => {
        return await GeneratedReport.filter(
  ...
  const generateScoreTable = useCallback(
  ...
  const capitalizePronounsInSentences = useCallback((htmlText) => {
    if (!htmlText) return "";
    ...
    async (assessment, resolvedTemplates) => {
      try {
        ...
        const assessmentData = await retryWithBackoff(
          async () => await Assessment.list("-created_date")
        );
        setAssessments(assessmentData);

        const allReports = await retryWithBackoff(
          async () => await GeneratedReport.list()
        );
